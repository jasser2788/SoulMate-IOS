...
class mapViewController: UIViewController {
    ...
    var maplatitude:Double = 35.38
    var maplongitude:Double = 9.32
    override func viewDidLoad() {
        
              super.viewDidLoad()
        mappView.delegate = self
        let gestureZ = UILongPressGestureRecognizer(target: self, action: #selector(self.longPress(_:)))
        mappView.addGestureRecognizer(gestureZ)
    ...
    @IBAction func pickLocation(_ sender: Any) {
     ...
     //  print(  mappView.centerCoordinate.latitude)
     //  print(  mappView.centerCoordinate.longitude)
    }
   
    @IBAction func longPress(_ sender: UILongPressGestureRecognizer) {
        if sender.state != UIGestureRecognizer.State.began { return }
            let touchLocation = sender.location(in: mappView)
            let locationCoordinate = mappView.convert(touchLocation, toCoordinateFrom: mappView)
        
            print("Tapped at lat: \(locationCoordinate.latitude) long: \(locationCoordinate.longitude)")
        latitude = locationCoordinate.latitude
        longitude = locationCoordinate.longitude
        annotation1.coordinate = CLLocationCoordinate2D(latitude:(locationCoordinate.latitude), longitude:locationCoordinate.longitude)
